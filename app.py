...
with st.sidebar:
    ...
    dt = st.slider(
        "Δt  Шаг времени",
        min_value=0.0005, max_value=0.008, value=0.002, step=0.0005,
        format="%.4f",
    )
    soft = st.slider(
    ...
    # render_skip default=5 per optimization requirement (was 2)
    render_skip = st.slider(
        "⚡ Шагов физики на кадр  (тепловой контроль)",
        min_value=1, max_value=12, value=5,
        help=(
            "Рекомендуется 5–10 для i7-1255U. "
            "Больше = быстрее физика, меньше нагрев, чуть менее плавно визуально."
        ),
    )
    refresh_s = st.slider(
        "🕐 Период обновления фрагмента (с)",
        min_value=0.03, max_value=1.0, value=0.06, step=0.01,
        format="%.2f",
        help="Как часто срабатывает heartbeat фрагмента. Меньше = плавнее, но тяжелее для CPU.",
    )
...
#  SIMULATION FRAGMENT
#
#  @st.fragment(run_every=refresh_s) causes Streamlit to call this function
#  on a timer without re-running the outer script.  Only the elements
#  created INSIDE the fragment are re-rendered; everything outside
#  (header, sidebar, metric slots, frame_slot) stays locked in the DOM.
#
#  We pass the pre-built slot references as arguments so the fragment can
#  write into the outer-script placeholders — this is the canonical pattern
#  for fragment-to-outer communication in Streamlit ≥ 1.33.
# ══════════════════════════════════════════════════════════════════════════════
...
def simulation_loop(
    ...
    """
    Heartbeat fragment — called every `refresh_s` seconds by Streamlit.

    Execution path
    ──────────────
    1. If running=False  → just paint the current state and return.
    2. If running=True   → advance physics `render_skip` steps,
                           render PNG, push to slots.

    No st.rerun(), no while loop, no blocking calls.
    The fragment's own re-invocation is handled entirely by Streamlit's
    internal scheduler via the run_every parameter.
    """
    # ── Status badge ─────────────────────────────────────────────────────────
    ...
    # ── Physics advance (only when running) ──────────────────────────────────
    if is_running:
        t0 = time.perf_counter()

        for _ in range(render_skip):
            st.session_state.pos, st.session_state.vel = leapfrog_step(
            ...
            st.session_state.t_sim += dt

        elapsed = time.perf_counter() - t0

        # FPS tracking — rolling 30-frame window stored in session_state
        ...
        # Thermal safety valve: if physics took > 400 ms we're thermal-throttling.
        # Yield the CPU briefly so the scheduler can breathe.
        if elapsed > 0.4:
            time.sleep(0.04)

    # ── Render ───────────────────────────────────────────────────────────────
    # Always render (even when paused) so the current state is visible.
    img_bytes = render_frame(
        st.session_state.pos,
        st.session_state.vel,
    ...
    # ── Push to pre-built DOM slots (in-place swap, no DOM insertion) ─────────
    frame_slot.image(img_bytes, use_container_width=True)

    fps_h = st.session_state.fps_history
    fps_v = f"{sum(fps_h)/len(fps_h):.1f}" if fps_h else "—"
    slot_particles.metric("ВСЕГО ЧАСТИЦ", f"{len(st.session_state.pos):,}")
    slot_step     .metric("ШАГ СИМУЛЯЦИИ", f"{st.session_state.step:,}")
    slot_fps      .metric("СКОРОСТЬ (FPS)",         fps_v)
    slot_time     .metric("ВРЕМЯ СИМ.",        f"{st.session_state.t_sim:.3f}")


# ── Invoke the fragment — registers the heartbeat with Streamlit ──────────────
simulation_loop(
    frame_slot     = frame_slot,
    slot_particles = slot_particles,
    slot_step      = slot_step,
    slot_fps       = slot_fps,
    slot_time      = slot_time,
